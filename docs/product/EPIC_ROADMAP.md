...
- **Implementation Order:**
  ...
  2. Upload + column-mapping UI (CSV/XLSX). **Not built** — Sprint D1's own scope excluded file parsing and any UI beyond a minimal read-only History list; the service-layer functions this UI will call (`createImportBatch()`, `ingestImportRows()`) already exist and were verified via a scratch script, not through this UI.
  3. Validation pass reusing existing Zod schemas — no new validation logic duplicated. **Orchestration built, no real validator** — `validateImportBatch()` exists and was verified live with a synthetic test validator; the `ImportRowValidator` contract a real `StudentImporter` etc. would implement (wrapping `registerStudentInputSchema` and friends) has no implementation yet.
  4. Preview (row-level diff, error surfacing) — mandatory gate before commit, never a direct upload-and-commit path. **Built, Sprint D1** — `previewImportBatch()`/`skipInvalidRows()`, returning the reusable `ImportReportDTO` (row counts + grouped error summary).
  5. Chunked, resumable commit (per [TRANSACTION_BOUNDARIES.md § 4](../database/TRANSACTION_BOUNDARIES.md#4-the-one-exception-academic-year-rollover-should-not-be-one-transaction)'s own precedent for large batch operations — an import is the same shape of problem as Academic Year Rollover, not the same shape as a 40-row attendance submission). **Built, Sprint D1** — `commitImportBatchChunk()`, one transaction per row (not per chunk), verified live to be genuinely resumable and to isolate one row's commit failure from its neighbors. No real commit handler exists yet (see item 3's caveat, same applies to `ImportRowCommitHandler`).
  6. Academic Structure import type (Classes/Sections/Subjects) — likely needed **before** Student/Teacher import types for a real client, since students need sections to enroll into. **Not built** — the first real consumer of items 3/5's extension points; not yet scoped as its own sprint.
