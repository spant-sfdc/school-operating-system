...
- **Core engine status: frozen, Sprint D4** ([D-046](./DECISIONS.md#d-046--sprint-d4-import-engine-freeze--architecture-certification-16-question-certification-against-real-code-not-assumed-one-genuine-gap-found-and-fixed-studentteacherattendanceservicets-gain-the-same-tx-passthrough-d2-gave-academicservicets-verified-live-with-a-real-rollback-test-two-narrower-composability-constraints-named-but-deliberately-not-fixed-epic-ds-core-engine-declared-frozen)) — `ImportBatch`/`ImportRow`, `src/services/import/`'s pipeline (Upload → Detect → Map → Validate → Preview → Commit → Audit → History → Report), and the `ImportRowValidator`/`ImportRowCommitHandler` extension-point contracts are certified against 16 mandatory architecture questions and will not change further for their own sake. A real, predicted gap (`student`/`teacher`/`attendance.service.ts` missing the `tx`-passthrough `academic.service.ts` already had) was found and fixed, live-verified with a rollback test. Two narrower composability constraints were found live and named, not fixed (real future plumbing for whichever sprint builds a Teacher/Attendance importer, avoidable today by design). Student/Teacher/Attendance/Result/Admission importers are each future work **against** this frozen engine — building one is a normal feature sprint, not a further engine-architecture sprint, unless that work discovers the engine itself is genuinely wrong (in which case, per this project's own discipline, it gets fixed and this freeze note is superseded, not silently overridden).
...
- **Implementation Order:**
  ...
  2. Upload + column-mapping UI (CSV/XLSX). **Built, Sprint D3** — `/admin/imports/upload` (CSV/XLSX parsing via `papaparse`/`exceljs` behind a format-agnostic `FileParser` abstraction, File Validation, SHA-256 duplicate detection) and `/admin/imports/mapping` (deterministic Detection, alias-dictionary-driven column mapping, Saved Mapping Template reuse, an Import Health Summary) — see [D-045](./DECISIONS.md#d-045--sprint-d3-upload-detection--smart-mapping-two-new-dependencies-papaparseexceljs-import-profiles-formalize-d2s-ad-hoc-aliases-into-a-reusable-registry-a-live-found-next_redirect-swallowed-by-trycatch-bug-fixed-in-the-mapping-action). ODS parsing remains a named, unbuilt gap.
  ...
  7. Student/Teacher/Attendance/Admission/Result import types — each its own future feature sprint against the now-frozen engine (item 6's reference architecture), not yet built. Student is the clearest next candidate: `registerStudent()`/`enrollStudent()` already have the `tx`-passthrough a commit handler needs (Sprint D4). Teacher/Attendance importers additionally need their commit handler to keep "create" and "assign/mark" as separate `ImportRow`s (a named, live-verified design constraint, not a blocker — see D-046). Result/Admission importers block on their own schema not existing yet (Epic E's Examination slice, Epic F's Admission slice, respectively) — a data-model gap, not an Import Engine gap.
...
- **Manual Verification Strategy:** A dry-run import against a deliberately messy sample spreadsheet (missing columns, duplicate admission numbers, a student assigned to a nonexistent section) — confirm every error surfaces at Preview, not mid-commit, and confirm a corrected re-upload commits cleanly and idempotently. **Now fully performable** — Sprint D3 built the Upload/Map UI and parsing library items 2 required; verified live via a real HTTP walkthrough (a genuine multipart file upload through the actual `/admin/imports/upload` page, through Mapping, through Commit) producing a real committed `SchoolClass`, not just a scratch-script simulation.
...
- **Implementation Order:**
  ...
  5. `PromotionRecord`/`TransferCertificate` schema (new migration) + end-of-year Promotion workflow (Admin) + Transfer/Withdrawal workflow (Admin) — fully specified at the documentation level already ([DOMAIN_MODEL.md § 6.3](../domain/DOMAIN_MODEL.md#63-promotionrecord)/[§ 6.4](../domain/DOMAIN_MODEL.md#64-transfercertificate), [BUSINESS_RULES.md § 6](../domain/BUSINESS_RULES.md#6-promotion--detention-policy)/[§ 7](../domain/BUSINESS_RULES.md#7-transfer--transfer-certificate)), not previously assigned to any Epic in this roadmap — found and fixed as a real documentation gap during Sprint E0's own certification ([D-047](./DECISIONS.md#d-047--sprint-e0-student-domain-architecture--workspace-certification-15-question-certification-against-real-code-and-domain-docs-a-genuine-createinclude-decomposition-risk-found-and-fixed-in-enrollmentrepositoryts-mirroring-d-046-a-real-epic_roadmapmd-gap-promotionrecordtransfercertificate-had-no-epic-home-fixed-student-architecture-declared-frozen)). Depends on 3–4 (rollover per [TRANSACTION_BOUNDARIES.md § 4](../database/TRANSACTION_BOUNDARIES.md#4-the-one-exception-academic-year-rollover-should-not-be-one-transaction)'s own supervised-batch design, not one transaction).
- **Estimated Complexity:** Medium (Attendance — mostly UI over an existing backend); Medium-High (Examination — new schema + new business rules for grade derivation); Medium (Promotion/Transfer — schema + workflow, business rules already fully specified).
