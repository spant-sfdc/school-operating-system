...
- **Implementation Order:**
  1. `ImportBatch` schema (new — a genuinely new entity, not yet in the domain model; tracks one import run's provenance).
  2. Upload + column-mapping UI (CSV/XLSX).
  3. Validation pass reusing existing Zod schemas — no new validation logic duplicated.
  4. Preview (row-level diff, error surfacing) — mandatory gate before commit, never a direct upload-and-commit path.
  5. Chunked, resumable commit (per [TRANSACTION_BOUNDARIES.md § 4](../database/TRANSACTION_BOUNDARIES.md#4-the-one-exception-academic-year-rollover-should-not-be-one-transaction)'s own precedent for large batch operations — an import is the same shape of problem as Academic Year Rollover, not the same shape as a 40-row attendance submission).
  6. Academic Structure import type (Classes/Sections/Subjects) — likely needed **before** Student/Teacher import types for a real client, since students need sections to enroll into.
